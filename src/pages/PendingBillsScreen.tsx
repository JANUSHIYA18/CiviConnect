...
const PendingBillsScreen = () => {
  ...
  useEffect(() => {
    ...
    apiRequest<{ pendingBills: PendingBill[] }>("/me/pending-bills")
      ...
      .catch((e) => setError(e instanceof Error ? e.message : "Could not load pending bills"));
  }, [user, navigate]);

  const totalAmount = useMemo(() => bills.reduce((sum, bill) => sum + bill.amount, 0), [bills]);

  return (
    <KioskLayout title="Pending Bills" subtitle="Outstanding Payments" showLogout>
        ...
        {!error && (
          <div className="space-y-6">
            <section className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-border bg-card p-5">
                <p className="text-sm text-muted-foreground">Nearest due date</p>
                <p className="text-2xl font-bold">{bills[0]?.dueDate || "N/A"}</p>
              </div>
            </section>
              ...
              {bills.map((bill) => (
                <article key={bill.billRef} className="rounded-2xl border border-border bg-card p-5">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-semibold uppercase">{bill.serviceType}</h3>
                  </div>
                  <p className="text-sm text-muted-foreground">Bill Reference: {bill.billRef}</p>
                  <p className="mt-3 flex items-center text-xl font-bold">
                    <IndianRupee className="mr-1 h-5 w-5" />
                    {bill.amount.toLocaleString("en-IN")}
                  </p>
                  <p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                    <CalendarClock className="h-4 w-4" /> Due by {bill.dueDate}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">Status: {bill.status}</p>
                </article>
              ))}
            </section>
          </div>
